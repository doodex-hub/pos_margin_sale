...
- **3 lesson penulisan Tour yang ditemukan sendiri (dicatat supaya tidak terulang):**
  ...
  4. **Regresi environment palsu (bukan bug kode):** setelah `docker compose down -v` (reset DB untuk `pin_message`), tour ini sempat kelihatan gagal lagi — modal "Open Register" tidak pernah ketemu. Screenshot (`docker cp` dari container exited) menunjukkan modal SEBENARNYA ADA, tapi ter-render Prancis ("Ouvrir la caisse") — akibat flag `--load-language=fr_FR` (dipasang sementara untuk verifikasi `MF-21`) yang bocor ke bahasa default seluruh sesi server, bukan cuma user tertentu. `cls.pos_admin.write({'lang': 'en_US'})` TIDAK cukup untuk fix ini. **Fix asli:** (a) `Dialog.confirm()` di tour ini diubah tanpa argumen teks (language-agnostic, sudah reflected di kode di atas), (b) flag `--load-language=fr_FR` dihapus permanen dari `docker-env/docker-compose.yml` (lihat komentar di file itu) — hanya dipasang manual sekali kalau `MF-21` perlu re-verifikasi.
- **Hasil akhir (re-run bersih, `docker compose down -v`, ketiga modul + `pin_message` tour sekaligus):** `tour succeeded` untuk modul ini, **0 failed, 1 error(s) of 19 tests** — 1 error itu `MF-21` (`sale_margin_threshold`, bahasa Prancis tidak terinstall di image dasar, bukan bug kode, lihat log modul itu), tidak terkait modul ini.
