...
## Addendum — Step 8 (Code Review), 2 temuan baru dari pembacaan kode langsung (2026-08-24)

> Ditemukan lewat gap-check Step 8 (bukan Tour test — Tour test yang sudah ada tidak pernah menyentuh dua jalur ini), dicatat di sini karena salah satunya butuh fix kode nyata, kembali ke Step 6.

- **`MF-24` (fix kode, wajib):** `static/src/js/pinMessage.js` — `condition` action "pins" memanggil `component.canAddReaction` sebagai property langsung di component Owl `Message`, pola valid di 17.0 tapi 18.0 memindahkan ini jadi method di message MODEL (`message.canAddReaction(thread)`). Akibatnya `condition` SELALU `false` — entry menu "Pin" tidak pernah render, di kondisi manapun, sejak awal migrasi (silent, tidak ada error). **Tidak ketahuan dari Tour test yang ada** karena tour sengaja pakai tombol inline `pinnedMessages.xml` (dipilih justru karena tombol action-menu ini "tidak pernah ketemu/timeout" saat penulisan tour — saat itu disangka flakiness Tour, ternyata tombolnya memang tidak pernah dirender). **Fix:** `condition` diubah jadi `component.message.canAddReaction(component.props.thread)` (idiom persis sama dengan core sendiri). **Re-verifikasi:** re-run `docker compose down -v` + test suite penuh — kedua Tour test (`pos_margin_threshold`, `pin_message`) tetap "tour succeeded", hasil akhir tidak berubah: `0 failed, 1 error(s) of 19 tests` (`MF-21`, tidak terkait). Detail lengkap: `FINDINGS.md` `MF-24`.
- **`MF-09` (konfirmasi, tanpa fix kode):** cabang `is_discussion` di `message.js`'s `onClickPin()` (full-override, warisan backfill `F-06`) dicek langsung terhadap source Odoo 17.0 DAN 18.0 (`docker run --rm odoo:17.0`/`pos_margin_sale_migration_18-odoo:latest`). Hasil: `messagePinService` yang dipakai adalah service CORE 17.0 asli (bukan buatan modul ini) — cabang ini murni duplikasi method core 17.0, bukan business logic custom. Di 18.0, `messagePinService` dihapus total DAN core tidak lagi memanggil `onClickPin()` untuk aksi native pin Discuss channel (native pakai `message.pin()` langsung di model) — cabang ini jadi dead code yang provably tidak terpanggil, TAPI Discuss channel pinning tetap berfungsi penuh lewat mekanisme native 18.0, tidak ada fitur yang hilang untuk end-user. Dibiarkan apa adanya (menghapus dead code ini di luar scope "port kode saja"). Detail lengkap: `FINDINGS.md` `MF-09`.
